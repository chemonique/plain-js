...
const showListContainer = document.getElementById('show-list');
 ...
 function fetchShowSeasons(showId) {
   ...
   fetch(`https://podcast-api.netlify.app/id/${showId}/seasons`)
     .then(response => response.json())
     .then(seasons => {
       const seasonItems = seasons.map(season => `
         <div>
           ...
           <button onclick="fetchSeasonEpisodes(${showId}, ${season.number})">View Episodes</button>
         </div>
       `).join('');
 ...
 function fetchSeasonEpisodes(showId, seasonNumber) {
   ...
   fetch(`https://podcast-api.netlify.app/shows/${showId}/seasons/${seasonNumber}/episodes`)
     .then(response => response.json())
     .then(episodes => {
 ...
 function toggleFavorite(episode) {
   const episodeIndex = favoriteEpisodes.findIndex(favEpisode => 
     favEpisode.id === episode.id && favEpisode.season === episode.season
   );

   if (episodeIndex === -1) {
     favoriteEpisodes.push(episode);
   } else {
     favoriteEpisodes.splice(episodeIndex, 1);
   }
 ...
 function updateShowList() {
   ...
   const showItems = filteredShows.map(show => `
     <div>
       ...
       <p>Last Updated: ${new Date(show.lastUpdated).toDateString()}</p>
